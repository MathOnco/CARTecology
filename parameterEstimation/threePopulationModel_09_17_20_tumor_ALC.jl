...
function CARmodel(du,u,p,t)
   ...
   (N, C, B) = u
	...
	rNfunc = rN
	rCfunc = rC*k1*B/(1.0 + k1*B) + b*(T - kN)^2/(a*T^2 + (T - kN)^2)
...
function my_loss_function(sol,params)
    ...
    (rN, rC, kC, k1,k2, a,b, gammaB) = params;
    ...
    #     tot_loss += lambda2*(T - ALC[n])^2/maxALC^2
	# end
...
function plotsol(CARmodel, result)
	...
	scatter!(fig,tALC,ALC,label=L"Normal_{data}")
...
lambda3 = 1e6
Nparams = 8
# Parameters: [rN, rC, kC, k1, k2, a,b, gammaB]
...
filename = "outputData_Tumor_ALC_dependence"
extension = ".txt"
...
write(myparamfile,"#rN, rC, kC, k1, k2, a, b, gammaB\n")
