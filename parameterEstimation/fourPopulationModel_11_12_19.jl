...
function CARmodel(du,u,p,t)
    ...
    # Grab the parameters
    # Parameters: (mu,rEff0,Gamma_E,Gamma_B,alpha_1,alpha_2)
    (mu,rEff0,Gamma_E,Gamma_B,alpha_1,alpha_2) = p
...
function my_loss_function(sol,params)
    ...
    (mu,rEff0,Gamma_E,Gamma_B,alpha_1,alpha_2) = params;
    ...
    EffBirth = rEff0*(1.0 + alpha_1*min(1.0,alpha_2))
...
function globalmethod(Nparams,bounds)
    ...
    result = bboptimize(cost_function2;SearchRange = bounds, MaxSteps = 2e4,Method=:de_rand_1_bin,TraceMode=:compact)
...
function plotsol(CARmodel, result)
    ...
    cb = nothing
    sol = solve(prob,Rodas4(autodiff=false),maxiters=1e9,force_dtmin=true,callback=cb,abstol=1e-12,reltol=1e-9)
...
write(myparamfile,"#(mu,rEff0,Gamma_E,Gamma_B,alpha_1,alpha_2)\n")
        ...
        for m = 2: 2 #1 : length(trial)
            for (indexLam,lambdas) in enumerate(lambdaSet)
                ...
                if ploton
                    ...
                    filename = string(directory,"fourPopulation_",string(counter),"_",Dates.today(),".pdf")
