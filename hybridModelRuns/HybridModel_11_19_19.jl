...
function runHybridModel(x0,initparams,sigma,stochasticThreshold)
    ...
    while t < tfinal
        ...
        (tnew,solContinuous) = continuousRegime(tspan,continuousPop,params)
        ...
        if saveTrajectory
            for i = 1 : length(tnew)
                append!(saveData,[tnew[i], solContinuous[1,i], solContinuous[2,i],solContinuous[3,i], solContinuous[4,i]])
            end
        end
        ...
        isDiscrete = continuousPop[4] - stochasticThreshold < 1e-11
        ...
        if discretePop[4] - tumorSizeThreshold*x0[4] >= -1e-5
            break
        end
...
sigmavec = [0.15]#[0.05;0.1;0.15]
B0prefactorvec = [1.0] #(0.5:0.5:20) #[0.01;0.1;1.0;2.0;5.0;10.0]
rBvec = (0.04:0.01:0.19)#[0.065] #(0.04:0.025:0.19)
...
for B0prefactor in B0prefactorvec
    for rB in rBvec
        ...
        header = @printf(myfile,"Initial conditions [N0,M0,E0,B0] = [%.3e,%.3e,%.3e,%.3e] with main parameters used to perturb with sigma = %.2e: rN = %.2e,rMmax = %.2e,rMmin = %.2e,tau = %.2e,kN = %.2e,kM = %.2e,mu = %.2e,rB = %.2e, rEff0 = %.2e, alpha_1 = %.2e, alpha_2 = %.2e ,Gamma_E = %.2e,Gamma_B = %.2e, stochastic threshold = %.0f\n",x0[1],x0[2],x0[3],x0[4],sigma,rN,rMmax,rMmin,tau,kN,kM,mu,rB,rEff0,alpha_1,alpha_2,Gamma_E,Gamma_B,stochasticThreshold)

        write(myfile,"time to event\tprogression or cure?\tparams\n--------------------------------------------------------------------------------------------------------------------------------------------\n")

        for n = 1 : nPatients

            (t,discretePop,continuousPop,pars) = runHybridModel(x0,initparams,sigma,stochasticThreshold)
